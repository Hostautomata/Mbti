...
<script type="text/javascript">
	 function write_go() {
/* 	location.href = "${pageContext.request.contextPath}/MyController?cmd=comunity_write&cPage=${pvo.nowPage}"; */	
					...
					<c:otherwise>
						<c:forEach var="k" items="${list}" varStatus="vs">
							<tr>
								...
								<td>${k.type }</td>
								<td><a href="/MyController?cmd=comunity_onelist&b_idx=${k.b_idx}&cPage=${pvo.nowPage}">${k.subject }</a></td>
								<td>${k.writer }</td>
							</tr>
						</c:forEach>
					</c:otherwise>
					...
					<td colspan="2" class="buttonline">
						<ol class="paging" >
							...
							<c:choose>
								...
								<c:otherwise>
									<li><a
										href="/MyController?cmd=comunity_list&cPage=${pvo.beginBlock-pvo.pagePerBlock}">이전으로</a></li>
								</c:otherwise>
							</c:choose>
							<!-- 블록안에 들어간 페이지번호들 -->
							<c:forEach begin="${pvo.beginBlock }" end="${pvo.endBlock }"
								...
								<c:choose>
									...
									<c:otherwise>
										<li><a
											href="/MyController?cmd=comunity_list&cPage=${k}">${k}</a></li>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							<!-- 다음 -->
							<c:choose>
								...
								<c:otherwise>
									<li><a
										href="/MyController?cmd=comunity_list&cPage=${pvo.beginBlock+pvo.pagePerBlock}">다음으로</a></li>
								</c:otherwise>
							</c:choose>
						</ol>
					</td>
